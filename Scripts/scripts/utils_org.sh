...
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/PARAM_DOMAIN/orderers/PARAM_ORDERER_HOSTNAME/msp/tlscacerts/tlsca.PARAM_DOMAIN-cert.pem

PEER0_PARAMORG_DOMAIN_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/PARAMORGDOMAIN/peers/peer0.PARAMORGDOMAIN/tls/ca.crt
...
setOrdererGlobals() {
  ...
  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/PARAM_DOMAIN/orderers/PARAM_ORDERER_HOSTNAME/msp/tlscacerts/tlsca.PARAM_DOMAIN-cert.pem
...
setGlobals() {
  ...
  ORG=$2
    ...
    CORE_PEER_LOCALMSPID="PARAMORG_DOMAINMSP"
    ...
    CORE_PEER_ADDRESS=peer0.PARAMORG_HOSTNAME:PARAM_PORT_1
...
fetchChannelConfig() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel fetch config config_block.pb -o PARAM_ORDERER_HOSTNAME:7050 -c $CHANNEL --cafile $ORDERER_CA
    set +x
  else
    set -x
    peer channel fetch config config_block.pb -o PARAM_ORDERER_HOSTNAME:7050 -c $CHANNEL --tls --cafile $ORDERER_CA
    set +x
  fi
