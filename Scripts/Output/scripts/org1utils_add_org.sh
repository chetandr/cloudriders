...
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/.example.com/msp/tlscacerts/tlsca.example.com-cert.pem

PEER0__CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/.example.com/peers/peer0..example.com/tls/ca.crt
...
setOrdererGlobals() {
  ...
  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/.example.com/msp/tlscacerts/tlsca.example.com-cert.pem
...
setGlobals() {
  ...
  ORG=$2
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0__CA
    ...
    CORE_PEER_ADDRESS=peer$PEER.org1$ORG.example.com:
...
fetchChannelConfig() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel fetch config config_block.pb -o .example.com:7050 -c $CHANNEL --cafile $ORDERER_CA
    set +x
  else
    set -x
    peer channel fetch config config_block.pb -o .example.com:7050 -c $CHANNEL --tls --cafile $ORDERER_CA
    set +x
  fi
